  ...
  <div class="wrapper">
    <div class="header">
      <mt-header :title="title">
        <router-link :to="backUrl+'?code='+detail.code" slot="left">
    ...
    <div class="footer-btn">
      <div class="total">
        ...
        <p class="account">(账户余额:{{ stockType==0? $store.state.userInfo.enableAmt:$store.state.userInfo.enableHmt}}元)</p>
      </div>
...
export default {
  ...
  mounted () {
    ...
    this.selectNumber = 0
    this.stockType = this.$route.params.stockType?this.$route.params.stockType:0;
    if(this.stockType == 1){
      this.title = "港股交易";
      this.backUrl="/listdetailhk"
    }
    //   this.selectCycle = 10
    this.getSettingInfo()
      ...
      if (!this.agree) {
        ...
        if(data!=null){
          ...
          if (+data.status === 0) {
            ...
            this.getUserInfo()
            if(this.stockType == 1){
              this.$router.push('/orderlist?index=5')
            }else{

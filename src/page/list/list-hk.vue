          ...
          <ul class="clearfix"
            ...
            <li class="li-title">
              ...
              <p class="code" style="padding-left: 0px">
                <i v-if="item.stock_plate != '科创' && item.stock_type
                  != 'HK'"
                   :class="item.stock_type == 'sz'?'iconfont shen-mark hushen-mark':'iconfont hushen-mark'">{{item.stock_type
                  == 'sz'?'深':'沪'}}</i>
                  <i v-else-if="item.stock_plate != '科创' && item.stock_type
                  == 'HK'" class="iconfont hk-mark">港</i>
            ...
            <li class="li-base">
              ...
              <span v-else>{{item.nowPrice-item.preclose_px>0 ?'+':''}} {{item.hcrate?Number(item.hcrate).toFixed(2):'0'}}%</span>
            </li>
...
export default {
  ...
  methods: {
    ...
    async getStock () {
      ...
      let data = await api.getStockGg(opt)
      if (data.status === 0) {

...
export default {
  ...
  created() {
    this.getDate();
    this.classStyle = localStorage.getItem("styleName")
  ...
  methods: {
    async getDate() {
      ...
      if (this.code.indexOf("hf_") != -1) {
        ...
        this.code.indexOf("sh") != -1 ||
        this.code.indexOf("sz") != -1
      ) {
        ...
        if (this.code.length === 5) {
           data = await api.getMinKEchartsGg(opts);
        } else {
           data = await api.getMinKEcharts(opts);
        }

  ...
  <div class="wrapper">
    ...
    <div class="forms">
      ...
      <div class="form-view">
        ...
        <input type="text" disabled v-model="invitecode" placeholder="输入机构代码">
      </div>
    </div>
    <div class="chebox">
      <i @click="isAgree"
...
import { isNull, isPhone, isEmail, pwdReg } from '@/utils/utils'
...
export default {
  ...
  methods: {
    ...
    async gook () {
      ...
      if (!this.agree) {
        Toast('需同意注册协议才能注册!')
      } else if (isNull(this.email) || !isEmail(this.email)) {
        Toast('请输入正确的邮箱')

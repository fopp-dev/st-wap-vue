...
export default {
  components: {},
  props: ["selectedNumberProp", "typeProp", "idProp"],
  data() {
    return {
  ...
  computed: {
    money() {
      return this.selectedNumberProp;
    }
  },
  created() {},
  mounted() {
    ...
    this.selectNumber = this.selectedNumberProp;
    this.getPayInfo();
  },
  beforeDestroy() {
  ...
  methods: {
    async getPayInfo() {
      ...
      let data = await api.getPayInfoDetail({ payId: this.id });
      if (data.status === 0) {

  ...
  <div class="wrapper">
    <div class="header">
      <mt-header title="忘记密码">
        <router-link to="/loginEmail" slot="left">
    ...
    <div class="form-group">
      ...
      <div class="page-part">
        <mt-field label="" type="text" placeholder="请输入邮箱" v-model="email"></mt-field>
...
export default {
  ...
  methods: {
    checkCodeBox () {
      if (isNull(this.email) || !isEmail(this.email)) {
        Toast('请输入正确的邮箱')
      } else {
        this.getcode();
      }
    ...
    async getcode () {
      ...
      if (isNull(this.email)) {
        ...
        if (!isEmail(this.email)) {
          ...
          let result = await api.getCodeEmail({ email: this.email })
          if (result.status === 0) {
            const TIME_COUNT = 60
            if (!this.timer) {
